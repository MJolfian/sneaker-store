...
function showRecievedSneakers(shoesArray){
	...
	shoesArray.forEach(item => {
		...
		img.alt = item.brand;
		div.classList = 'product-box flex flex-col cursor-pointer';
		div.dataset.pid = item.pid; // or: div.setAttribute('data-pid', item.pid);
...
containerOfProductsSection.addEventListener('click', (event) => {
		if(!event.target.closest('.product-box')) return;
